...
function drawUI(step) {
  ...
  if (game.statusMessageTime > 0) {
    drawOutlinedText(game.statusMessage, 5, fieldHeight - 20, 'left');
    game.statusMessageTime -= step;
  } else if (game.state === gameState.LEVEL_STARTING) {
...
function updateGame() {
  ...
  if (game.state === gameState.LEVEL_PLAYING) {
    ...
    if (game.time >= levelTimeLimit) {
      ...
      Platform.speed = 0;
      submitScore(game.previousScore + game.score);
    }
...
function submitSave() {
  var msg = {
    ...
    'gameState': {
      'level': game.level,
      'score': game.previousScore
    }
  };
  window.parent.postMessage(msg, '*');
  game.statusMessage = 'Game saved';
  game.statusMessageTime = tempMessageTime;
}
...
function processLoadData(data) {
  if (data.messageType === 'LOAD') {
    game.level = data.gameState.level;
    game.previousScore = data.gameState.score;
    initLevel();
    game.changeState(gameState.LEVEL_STARTING);
    game.score = 0;
    game.statusMessage = 'Loaded saved game';
    game.statusMessageTime = tempMessageTime;
  } else if (data.messageType === 'ERROR') {
    game.statusMessage = data.info;
    game.statusMessageTime = tempMessageTime;
  }
}
